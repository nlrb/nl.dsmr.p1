const crc   = require('../node-crc/lib');
...
const object_patterns = {
  ...
  'gas timestamp' : {
    pattern   : /^0-[0-9]+:24\.2\.1\(([0-9.]+[WS])\)/m, // DSMR v4
    pattern   : /^0-[0-9]+:24\.3\.0\(([0-9.])\)/m,      // DSMR v3
	transform : utils.parseTimestamp
  },
  'gas delivered' : {
    pattern   : /^0-[0-9]+:24\.2\.1\([0-9.]+[WS]\)\(([0-9.]+)/m, // DSMR v4
	pattern   : /^\(([0-9.]+)/m,                                 // DSMR v3  
    transform : Number
  },
};

module.exports = {
  parse: function(telegram, settingsdsmrV4) {
    return this.validate(telegram, settingsdsmrV4);
  },
  validate: function(telegram, settingsdsmrV4) {
    return Telegram(telegram, settingsdsmrV4);
  }
};

function Telegram(telegram, settingsdsmrV4) {
  if (! (this instanceof Telegram)) return new Telegram(telegram, settingsdsmrV4);
  this.telegram = telegram;
  this.parse(settingsdsmrV4);
};

Telegram.prototype.parse = function(settingsdsmrV4) {
  dsmrV4 = settingsdsmrV4;
  if (dsmrV4) { 
    var match = this.telegram.match(patterns.TELEGRAM); // DSMR v4

    // Validate the overall telegram structure (for DSMR v4 only).
    if (! match) throw new Error('INVALID_TELEGRAM');
  
    // Validate the checksum (for DSMR v4 only).
    var crc = this.checksum()
    if (crc !== match[3]) throw new Error('CHECKSUM_MISMATCH (' + crc + ',' + match[3] + ')');

    // Parse properties and data for DSMR 4.
    this.identifier = match[1];
    this.crc        = match[3];
    this.objects    = this.parseObjects(match[2]);
  } else {  
      // Parse properties and data for DSMR 3. Note that no checks are done on the telegram its formatting !!
      var match = this.telegram;
	  this.identifier = match;
      this.crc        = null; // DSMR 3 does not have / uses a CRC
      this.objects    = this.parseObjects(match);
  }
};
