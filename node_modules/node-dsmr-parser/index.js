...
Telegram.prototype.parse = function() {
  var match_v4 = this.telegram.match(patterns.TELEGRAMv4);

  // Validate the overall telegram structure for v4
  if (match_v4) {
	  // Validate the checksum.
      var crc = this.checksum()
	  if (crc !== match_v4[3]) throw new Error('CHECKSUM_MISMATCH (' + crc + ',' + match_v4[3] + ')');

      // Parse properties and data
      this.crc = match_v4[3];
      this.identifier = match_v4[1];
      this.objects = this.parseObjects(match_v4[2]);
  } else {
      var match_v3 = this.telegram.match(patterns.TELEGRAMv3);
      // Validate the overall telegram structure for v4
	  if (!match_v3) throw new Error('INVALID_TELEGRAM');

      // Parse properties and data.
      this.identifier = match_v3[1];
      this.objects = this.parseObjects(match_v3[2]);
	  
	  // Remove v3 specific names
	  for (var key in this.objects) {
		  var p = key.indexOf(' v3');
		  if (p > 0) {
			  this.objects[key.slice(0, p)] = this.objects[key];
			  delete this.objects[key];
		  }
	  }
	  this.objects['dsmr version'] = '< 4';
  }
};
