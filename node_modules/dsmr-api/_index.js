...
var self = module.exports = {
	...
	addMeter: function(driver, device, settings, name) {
		// use ip:port as temporary id if serial number not known yet
		let id = device == null ? settings.ip + ':' + settings.port : device.id;
		if (meters[id] == null) {
			let dsmr = new DSMR(settings, debug);
			if (device != null) { // ID is correct
				meters[id] = dsmr;
				meters[id].realSum = { dsmr: { ready: false }, producers: { ready: false } };
				meters[id].name = name;
				sensors[id] = [];
	...
	addMeterActions: function(driver, device) {
		...
		meters[id].events.on('newMessage', function() {
			if (meters[id].settings.compensate) {
				debug('Requesting producer sum', id);
				meters[id].realSum = { dsmr: { ready: false, value: 0 }, producers: { ready: false, value: 0 } };
				self.getProducerSum(id, function(produced) {
					debug('Sum received: ' + produced);
					meters[id].realSum.producers = { ready: true, value: produced };
					meters[id].events.emit('actualWatt', meters[id].realSum.producers);
				});
			} else {
				debug('No compensation for', id);
				meters[id].realSum = { dsmr: { ready: false, value: 0 }, producers: { ready: true, value: 0 } };
			}
		});
		meters[id].events.on('actualWatt', function(newVal) {
			if (typeof newVal != 'object') {
				meters[id].realSum.dsmr = { ready: true, value: newVal };
			}
			if (meters[id].realSum.dsmr.ready && meters[id].realSum.producers.ready) {
				meters[id].values.measuredWatt = meters[id].realSum.dsmr.value + meters[id].realSum.producers.value;
		...
		meters[id].events.on('flowGas', function(newVal) {
			driver.realtime(device, 'flow_gas', newVal, function(err, success) {
				debug('Real-time meter gas flow update: ' + (err ? err : 'OK'));
			});
		});
	...
	updateMeterName: (id, name) => meter[id].name = name,
